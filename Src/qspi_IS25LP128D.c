...
static QSPI_AutoPollingTypeDef polling;
...
static uint32_t auto_polling(uint32_t time)
{
  cmd.InstructionMode = qspi_mode ? QSPI_INSTRUCTION_4_LINES : QSPI_INSTRUCTION_1_LINE;
  ...
  polling.StatusBytesSize = 1;
  polling.Interval        = 0x10;
  polling.AutomaticStop   = QSPI_AUTOMATIC_STOP_ENABLE;

  if (HAL_QSPI_AutoPolling(&hqspi, &cmd, &polling, time) != HAL_OK)
      return 1;
